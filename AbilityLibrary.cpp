...
AbilityLibrary::AbilityLibrary(SuperChessGame& game_, Logger& log_)
{
    ...
    lib[std::make_pair(Piece::Queen, Tier::T0)] = std::make_unique<QueenKamikaze>(game_, log_);
    ...
    lib[std::make_pair(Piece::Queen, Tier::T1)] = std::make_unique<QueenTier0>(game_, log_);
}
